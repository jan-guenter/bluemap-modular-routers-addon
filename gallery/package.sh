...
archive_temp="$(mktemp -d /tmp/bluemap-modular-routers-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/modular-routers-gallery.zip" -@
)
unzip -tq "$archive_temp/modular-routers-gallery.zip"
cp "$archive_temp/modular-routers-gallery.zip" "$output_path"
